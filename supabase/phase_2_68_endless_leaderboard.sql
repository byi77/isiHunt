-- Phase 2.68: Kumulierte Endlos-Gates und eigene Endlos-Rangliste.
-- Bereits gebuchte Runden bleiben unveraendert; die Rangliste liest nur servergepruefte Belege.
begin;

do $$ begin
  if not exists (select 1 from public.isihunt_schema_state
                 where singleton = true and schema_version in (67, 68)) then
    raise exception 'Phase 2.67 muss vor Phase 2.68 ausgefuehrt werden';
  end if;
end $$;
...
begin
  ...
  if p_round > 1 then
    ...
    select sum(case gate.round_number
      ...
      else 1300 + (gate.round_number - 5) * 300 end)::integer
    into previous_gate
...
create or replace function public.get_endless_leaderboard(p_limit integer default 50)
returns table (
...
set schema_version = 68, migration_name = 'phase_2_68_endless_leaderboard.sql', applied_at = now()
