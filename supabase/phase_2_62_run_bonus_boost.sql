...
-- Phase 2.62: Leistungsabhaengige Abschlussboni deutlich erhoehen.
--
-- Die garantierte Abschlussprämie bleibt unveraendert. Seltene Beute, Serien
-- und Sammelmengen zahlen jeweils doppelt so viel wie in Phase 2.61.
...
begin
  if not exists (
    ...
    where singleton = true and schema_version in (61, 62)
  ) then
    raise exception 'Phase 2.61 muss vor Phase 2.62 ausgefuehrt werden';
  end if;
end;
$$;
...
set schema_version = 62,
    migration_name = 'phase_2_62_run_bonus_boost.sql',
    applied_at = now()
