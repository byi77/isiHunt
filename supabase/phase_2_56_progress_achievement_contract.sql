-- Phase 2.56: Erfolgsbedingungen und Erfolgs-Coins an den Clientvertrag angleichen.
--
-- Phase 2.54 uebergab `safe_best_combo` als Laufgesamtmenge. Dadurch konnten
-- clean_run_* mit einer Combo statt mit der Fangzahl geprueft werden. Zudem
-- verwendete die SQL-Coinformel `rank` statt `rank - 1`; Rang 1 war damit zu
-- hoch verguetet. Diese Migration aendert nur die zukuenftige Buchung.
...
begin
  if not exists (
    ...
    where singleton = true and schema_version in (55, 56)
  ) then
    raise exception 'Phase 2.55 muss vor Phase 2.56 ausgefuehrt werden';
  end if;
end;
$$;
...
set schema_version = 56,
    migration_name = 'phase_2_56_progress_achievement_contract.sql',
    applied_at = now()
