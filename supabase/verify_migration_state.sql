...
-- Erwartet genau eine Zeile mit schema_version = 67.
...
begin
  if not exists (
    ...
    where singleton = true and schema_version = 67
  ) then
    raise exception 'Erwartet schema_version = 67';
  end if;
end;
$$;
...
where n.nspname = 'public'
  and p.proname in (
    'submit_progress_event', 'submit_endless_round', 'claim_daily_bonus', 'redeem_sync_code',
