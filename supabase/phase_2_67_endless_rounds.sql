-- Phase 2.67: Serverseitig gepruefte Endlos-Runden und 30s-Plausibilitaet.
-- Gleiche Profilbuchung wie Solo, eigene Laufdauer und temporaere Talente.
begin;

do $$ begin
  if not exists (select 1 from public.isihunt_schema_state
                 where singleton = true and schema_version in (66, 67)) then
    raise exception 'Phase 2.66 muss vor Phase 2.67 ausgefuehrt werden';
  end if;
end $$;

alter table public.profile_progress_events
  add column if not exists endless_session_id uuid,
  add column if not exists endless_round integer,
  add column if not exists endless_talents jsonb;

create unique index if not exists profile_progress_events_endless_round_unique
  on public.profile_progress_events(profile_id, endless_session_id, endless_round)
  where endless_session_id is not null;
...
begin
  ...
  if p_round > 1 then
    select score, endless_talents into previous_score, previous_talents
  ...
  ) * (1 + greatest(0, p_round - 1) * 0.02) then
...
set schema_version = 67, migration_name = 'phase_2_67_endless_rounds.sql', applied_at = now()
